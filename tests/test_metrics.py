...
from metrics import compute_edgegrad_mean
from metrics import compute_laplacian_clarity
from metrics import compute_region_metrics
from metrics import compute_sigma_c
from metrics import compute_sigma_l


def test_compute_sigma_l_uses_grayscale_std_inside_mask():
    ...
    mask[:2, :2] = True
    assert compute_sigma_l(image, mask) == pytest.approx(85.3185213657655)


def test_compute_sigma_c_uses_chroma_channels_inside_mask():
    image = np.zeros((4, 4, 3), dtype=np.uint8)
    image[:2, :2] = np.array(
        [
            [[255, 0, 0], [0, 255, 0]],
    ...
    mask = np.zeros((4, 4), dtype=bool)
    mask[:2, :2] = True
    assert compute_sigma_c(image, mask) > 0.0


def test_compute_laplacian_clarity_detects_edge_content():
    image = np.zeros((8, 8, 3), dtype=np.uint8)
    image[:, 4:] = 255
    assert compute_laplacian_clarity(image) > 0.0


def test_compute_edgegrad_mean_detects_edge_content():
    image = np.zeros((8, 8, 3), dtype=np.uint8)
    image[:, 4:] = 255
    assert compute_edgegrad_mean(image) > 0.0
...
def test_compute_region_metrics_with_none_mask_uses_full_image():
    ...
    assert result["sigma_L"] == pytest.approx(0.0)
...
def test_compute_region_metrics_preserves_coverage_for_tiny_invalid_mask():
    ...
    result = compute_region_metrics(image, mask)
    assert result["valid"] is False
    assert result["Laplacian_Clarity"] == 0.0
    assert result["EdgeGrad_mean"] == 0.0
    assert result["sigma_L"] == 0.0
